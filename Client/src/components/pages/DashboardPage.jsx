...
//       const { tasks, notifications, user: userData } = response.data;
...
//           completedDays: userData.streaks?.map(streak => !!streak) || new Array(21).fill(false),
//           completedDates: userData.streaks?.map(streak => streak?.date) || new Array(21).fill(null),
...
//           </div>
//           <div className="grid grid-cols-7 gap-2">
//             {Array.from({ length: dashboardData.streaks.totalDays }).map((_, index) => (
//               <div
//                 key={index}
//                 className={`aspect-square rounded-lg flex items-center justify-center relative
...
//                     : 'bg-gray-700/30 border-2 border-transparent hover:border-gray-600'
//                   }
//                   ${!dashboardData.streaks.completedDays[index] && index === dashboardData.streaks.completedDays.findIndex(day => !day) && !hasCompletedToday()
//                     ? 'cursor-pointer hover:bg-gray-700/50'
//                     : 'cursor-not-allowed'
//                   }
//                   transition-all duration-200`}
//                 onClick={() => handleDailyStreakCompletion(index)}
//                 title={dashboardData.streaks.completedDays[index]
//                   ? `Completed on ${new Date(dashboardData.streaks.completedDates[index]).toLocaleDateString()}`
//                   : index === dashboardData.streaks.completedDays.findIndex(day => !day) && !hasCompletedToday()
//                     ? "Click to complete today's challenge"
//                     : "Not available yet"
